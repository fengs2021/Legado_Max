...
> 在 `<useweb>` 页面脚本中可直接使用注入的 `java`、`source`、`cache`。\
> 按钮点击直接调用页面或 `jsLib` 中的函数即可，例如 `<button onclick="set()">刷新</button>`。
...
> 这种写法适合把多个排行、专区、标签筛选做成一个自定义发现入口页。\
> 普通分类列表建议直接写发现 url 或发现 url JSON，会更简单。

> `<useweb>` 只是入口页面；按钮打开的列表仍然使用本书源的发现规则解析，所以列表规则、书名、作者、封面、详情页规则照常填写。\
> 按钮通常调用 `java.open("explore", url, title)`：`explore` 表示打开发现列表，`url` 是要解析的列表地址，`title` 是打开后的页面标题。\
> 列表地址需要分页时保留 `{{page}}` 占位，例如 `网址+{{page}}`。中文、标签等参数建议用 `encodeURIComponent()` 编码。\
> `source.key` 可用于获取当前书源地址。页面会自动注入 `java`、`source`、`cache` 和当前源的 `jsLib`，可以用来保存筛选状态、读取源变量或复用函数。

> 模板：填入`发现地址规则（url）`，修改 `items` 中的标题和地址即可。
